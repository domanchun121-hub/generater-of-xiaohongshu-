...
<head>
    ...
    <title>RedNote Lite - 阿泽定制版 v8.3</title>
    ...
    <link
        href="https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@300;400;500;700;900&family=Noto+Serif+SC:wght@300;400;600;700&family=Inter:wght@400;600;800&display=swap"
        rel="stylesheet">
...
<body class="flex h-screen w-full text-gray-800 select-none overflow-hidden">
    ...
    <aside id="left-panel"
        ...
        <div class="h-12 border-b border-gray-100 flex items-center justify-between px-4 shrink-0 bg-white">
            <h2 class="font-bold text-sm flex items-center gap-2"><i data-lucide="pen-tool" size="14"></i> 正文编辑</h2>
            <button onclick="pasteFromClipboard()"
                ...
                title="读取剪贴板">
                <i data-lucide="clipboard-paste" size="12"></i> 粘贴
            </button>
        </div>

        <div class="flex-1 p-4 flex flex-col h-full overflow-hidden">
            <div class="mb-2 flex justify-between items-center">
                <span class="text-xs text-gray-400">Markdown / 图片粘贴(Ctrl+V)</span>
                <label
                    ...
                    <i data-lucide="image-plus" size="12"></i> 插入图片
                    <input type="file" accept="image/*" class="hidden" onchange="handleBodyImageUpload(this)">
                </label>
            </div>
            ...
            <textarea id="input-content"
                ...
                oninput="updatePreview()" spellcheck="false">
# 这是一个一级标题

## 这是一个二级标题

这里是正文段落。如果需要强调，可以使用**加粗文字**。
...
**4. 如何插入图片？**
- 方法A：直接截图，在这里按 Ctrl+V 粘贴
- 方法B：点击上方的“插入图片”按钮
</textarea>
            <div class="mt-2 text-[10px] text-gray-400 text-center">输入 @ + --- 可强制分页</div>
        </div>

        <div id="left-toggle" class="toggle-btn" onclick="togglePanel('left')" title="折叠/展开">
    ...
    <main class="flex-1 bg-[#E5E5E5] relative overflow-hidden flex flex-col group/preview min-w-0">

        <div class="absolute bottom-6 right-6 z-30 flex flex-col items-end pointer-events-none">
            ...
            <div id="toolbar-actions"
                ...
                <div class="flex items-center bg-gray-50 rounded-lg p-1">
                    <button onclick="changeZoom(-0.1); hideToolbar();"
                        class="w-8 h-8 flex items-center justify-center hover:bg-white rounded-md text-gray-600 shadow-sm active:scale-90"><i
                            data-lucide="minus" size="16"></i></button>
                    <span id="zoom-val" class="w-10 text-center font-mono text-xs font-bold text-gray-600">100%</span>
                    <button onclick="changeZoom(0.1); hideToolbar();"
                        class="w-8 h-8 flex items-center justify-center hover:bg-white rounded-md text-gray-600 shadow-sm active:scale-90"><i
                            data-lucide="plus" size="16"></i></button>
                </div>
                <button onclick="toggleLayout(); hideToolbar();" id="layout-btn"
                    ...
                    <i data-lucide="layout-grid" size="16"></i> <span>切换视图</span>
                </button>
            </div>

            <button onclick="toggleToolbar()" id="toolbar-toggle"
        ...
        <div id="scroll-container" class="flex-1 overflow-y-auto p-8 w-full custom-scrollbar flex justify-center">
            <div id="preview-canvas"
                class="flex flex-col items-center gap-8 origin-top transition-transform duration-200 pb-32"></div>
        </div>
    </main>
    ...
    <aside id="right-panel"
        ...
        <div class="h-12 border-b border-gray-100 flex items-center justify-between px-4 shrink-0 bg-white">
            <div class="flex items-center gap-2">
                <div class="w-6 h-6 bg-black rounded flex items-center justify-center text-white font-bold text-xs">R
                </div>
                <h1 class="font-bold text-sm">参数设置</h1>
            </div>
            <button onclick="downloadAll()"
                ...
                <i data-lucide="download" size="12"></i> 导出JPG
            </button>
        </div>
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar">
            ...
            <div>
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">风格主题</h3>
                <div class="grid grid-cols-2 gap-2" id="style-grid"></div>
            </div>

            <div class="space-y-4 pt-4 border-t border-gray-100">
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest">封面信息</h3>
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-lg p-2">
                    <div class="flex items-center gap-2">
                        <label
                            ...
                            <i data-lucide="image" size="12"></i> 换图
                        ...
                        <button onclick="clearImage()"
                            ...
                            title="移除图片"><i data-lucide="trash-2" size="14"></i></button>
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1">封面大标题</label>
                    <input type="text" id="input-title"
                        ...
                        value="高效育儿心得" oninput="updatePreview()">
                </div>
                <div>
                    <label class="block text-xs font-bold text-gray-500 mb-1">封面简介</label>
                    <textarea id="input-intro" rows="2"
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div><label class="block text-xs font-bold text-gray-500 mb-1">日期</label><input type="text"
                            id="input-date" class="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs"
                            value="VOL.08" oninput="updatePreview()"></div>
                    <div><label class="block text-xs font-bold text-gray-500 mb-1">作者</label><input type="text"
                            id="input-tag" class="w-full bg-gray-50 border border-gray-200 rounded px-2 py-1.5 text-xs"
                            value="@常用名" oninput="updatePreview()"></div>
                </div>
            </div>

            <div class="space-y-4 pt-4 border-t border-gray-100">
                <h3 class="text-xs font-black text-gray-400 uppercase tracking-widest">外观参数</h3>

                <div>
                    <label class="text-xs font-bold text-gray-600 mb-1 block">画布尺寸</label>
                    <div class="relative">
                        <select id="canvas-size"
                            ...
                            <option value="375x500">📕 小红书 (3:4)</option>
                            <option value="375x667">📱 手机海报 (9:16)</option>
                        </select>
                        <i data-lucide="chevron-down" size="14"
                            class="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none"></i>
                    </div>
                </div>

                <div class="space-y-4">
                    <div>
                        <div class="flex justify-between text-xs text-gray-500 mb-1"><span>封面字号</span><span
                                id="title-size-val">48px</span></div>
                        <input type="range" id="title-size" min="30" max="80" value="48"
                            class="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-black"
                            oninput="updatePreview()">
                    </div>

                    <div>
                        <div class="flex justify-between text-xs text-gray-500 mb-1"><span>正文字号</span><span
                                id="body-size-val">15px</span></div>
                        <input type="range" id="body-size" min="8" max="24" value="15"
                            class="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-black"
                            oninput="updatePreview()">
                    </div>

                    <div class="pt-2 border-t border-gray-100">
                        <div class="flex justify-between text-xs text-gray-500 mb-1"><span>标题整体 (H1-H4)</span><span
                                id="heading-scale-val">26px</span></div>
                        <input type="range" id="heading-scale" min="20" max="60" value="26"
                            class="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-black"
                            oninput="updatePreview()">
                    </div>
                </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 text-center pb-6">
                <p class="text-[10px] text-gray-400 font-mono leading-relaxed opacity-70">
                    小红书笔记生成器<br>
                    Customized for <span class="font-bold text-gray-500">@阿泽的育儿书架</span><br>
                    Powered by Google Gemini
                </p>
            </div>
        </div>

        <div id="right-toggle" class="toggle-btn" onclick="togglePanel('right')" title="折叠/展开">
    ...
    <script>
        // 隐藏彩蛋
        console.log("%c RedNote Lite | Customized for @阿泽的育儿书架 | Powered by Google Gemini", "background: #000; color: #fff; padding: 4px; border-radius: 4px;");

        const STYLES = [
            // 更新了浅蓝色的配置
            { id: 'minimal', name: '🔵 浅蓝风', class: 'theme-minimal', bg: '#EBF5FB', text: '#1E3A5F' },
            { id: 'serif', name: '📖 书卷风', class: 'theme-serif', bg: '#F2EFE9', text: '#2c2c2c' },
            { id: 'acid', name: '⚡ 冲击波', class: 'theme-acid', bg: '#CCFF00', text: '#000' },
            { id: 'tech', name: '🔵 科技风', class: 'theme-tech', bg: '#0F172A', text: '#FFF' },
        ];
        ...
        function setupResizer(resizerId, panelId, side) {
            ...
            const onMove = (e) => {
                ...
                let newWidth = side === 'left' ? clientX : window.innerWidth - clientX;
        ...
        function initUrlParams() {
            ...
            if (urlParams.has('title')) document.getElementById('input-title').value = decodeURIComponent(urlParams.get('title'));
        ...
        function updatePreview() {
            ...
            container.appendChild(createCardWithDownload(renderCoverHTML(styleConfig, title, date, tag, intro), `rednote_cover.jpg`));

            pages.forEach((pageText, index) => {
                if (!pageText.trim()) return;
                let processedText = pageText.replace(/::: row\n([\s\S]*?)\n:::/g, '<div class="img-row">$1</div>');
                const htmlContent = marked.parse(processedText);
                container.appendChild(createCardWithDownload(renderPageHTML(styleConfig, htmlContent, index + 1, date), `rednote_page_${index + 1}.jpg`));
            });
        ...
        function autoFitText() {
            const cards = document.querySelectorAll('.card-wrapper.auto-fit-page');
            cards.forEach(card => {
                const markdownBody = card.querySelector('.markdown-body');
                if (!markdownBody) return;
                
                // Keep shrinking until scrollHeight is <= clientHeight (with a 20px padding buffer)
                let currentSize = parseFloat(window.getComputedStyle(markdownBody).fontSize);
                const minSize = 10; // Don't go below 10px
                
                while(markdownBody.scrollHeight > markdownBody.clientHeight + 2 && currentSize > minSize) {
                    currentSize -= 0.5;
                    markdownBody.style.fontSize = currentSize + 'px';
                }
            });
        }
        ...
        async function downloadAll() {
            ...
            try {
                ...
                for (let i = 0; i < cards.length; i++) {
                    ...
                    zip.file(`rednote_page_${i + 1}.jpg`, imgData, { base64: true });
                }
        ...
        function renderCoverHTML(config, title, date, tag, intro) {
            if (state.coverImage) {
                let themeStyles = {
                    // 更新了浅蓝色的封面配置
                    'minimal': { bg: '#EBF5FB', text: '#566573', title: '#1E3A5F', border: '#3498DB', fontTitle: "'Inter', sans-serif", fontBody: "'Inter', sans-serif" },
                    'serif': { bg: '#F2EFE9', text: '#2c2c2c', title: '#2c2c2c', border: '#B85C5C', fontTitle: "'Noto Serif SC', serif", fontBody: "'FangSong', serif" },
        ...
        function renderPageHTML(config, html, pageNum, date) {
            if (config.id === 'minimal') {
                return `<div class="card-bg auto-fit-page relative flex flex-col h-full"><div class="flex justify-between items-center mb-6 pb-4" style="border-bottom: 1px solid #AED6F1;"><span class="text-xs font-bold tracking-widest" style="color: #AED6F1;">${date}</span><span class="font-bold text-xs px-2 py-1 rounded" style="background-color: #D6EAF8; color: #1E3A5F;">0${pageNum}</span></div><div class="markdown-body flex-1 overflow-hidden" style="font-size: ${state.bodySize}px">${html}</div></div>`;
            }
            if (config.id === 'serif') return `<div class="card-bg auto-fit-page flex flex-col"><div class="flex justify-between border-b border-gray-300 pb-2 mb-2 text-xs text-gray-400 font-bold tracking-widest"><span>${date}</span><span>第 ${pageNum} 页</span></div><div class="markdown-body flex-1 overflow-hidden" style="font-size: ${state.bodySize}px">${html}</div></div>`;
            if (config.id === 'acid') return `<div class="card-bg auto-fit-page flex flex-col"><div class="flex justify-between border-b-2 border-black pb-2 mb-2 font-mono text-xs font-bold"><span>${date}</span><span>P.0${pageNum}</span></div><div class="markdown-body flex-1 overflow-hidden" style="font-size: ${state.bodySize}px">${html}</div></div>`;
            return `<div class="card-bg auto-fit-page flex flex-col"><div class="markdown-body flex-1 overflow-hidden" style="font-size: ${state.bodySize}px">${html}</div></div>`;
        }
