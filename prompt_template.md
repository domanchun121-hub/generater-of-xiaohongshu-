...
直接输出润色后的终稿正文，绝对禁止输出“风格特征摘要”、“AI特征分析”、“核心优化策略”等任何前置分析内容。

正文必须使用 Markdown 格式，并且：

禁止使用任何标题符号（#、##），只需使用加粗突出核心词元或金句即可。

(关键排版要求) 必须有意识地控制每页的视觉容量，每张画布（即两个 @--- 之间的内容）字数应严控在 100-150 字左右。

(软分行与留白规则)

每行末尾使用两个空格 + 回车（即 Markdown 软分行）实现紧凑换行，让上下两行逻辑紧密但视觉上分行显示。
